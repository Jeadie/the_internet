...
urlpatterns = [
    ...
    re_path(
        f'^(?P<month>({"|".join(InternetNewsIndexView.allowed_url_months)}))/$',
        InternetNewsIndexView.as_view(),
        name='news_month'
    ),
    re_path(
        f'^(?P<month>({"|".join(InternetNewsIndexView.allowed_url_months)}))/' + '(?P<day>[0-9]{1,2})/$',
        InternetNewsIndexView.as_view(),
        name='news_day'
    ),
